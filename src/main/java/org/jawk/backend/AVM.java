...
import org.jawk.jrt.JRT;	// ONLY for DEFAULT_RS_REGEX
...
public class AVM implements AwkInterpreter, VariableManager {
		...
		while(! position.isEOF()) {
			...
			switch(opcode) {
			  ...
			  case AwkTuples._NOT_: {
				...
				Object o = pop();
				boolean result;
				if (o instanceof Integer) result = ((Integer)o).intValue() != 0;
				else if (o instanceof Double) result = ((Double)o).doubleValue() != 0;
				else if (o instanceof String) result = (o.toString().length()>0);
				else {
			  ...
			  case AwkTuples._POW_EQ_ARRAY_:
			  {
				...
				switch(opcode) {
				  case AwkTuples._PLUS_EQ_ARRAY_: new_val = orig_val + val; break;
				  case AwkTuples._MINUS_EQ_ARRAY_:new_val = orig_val - val; break;
				  case AwkTuples._MULT_EQ_ARRAY_:new_val = orig_val * val; break;
				  case AwkTuples._DIV_EQ_ARRAY_:new_val = orig_val / val; break;
				  case AwkTuples._MOD_EQ_ARRAY_:new_val = orig_val % val; break;
				  case AwkTuples._POW_EQ_ARRAY_:new_val = Math.pow(orig_val, val); break;
				  default: throw new Error("Invalid op code here: "+opcode);
				}
			  ...
			  case AwkTuples._POW_EQ_:
			  {
				...
				double ans;
				switch(opcode) {
				  case AwkTuples._PLUS_EQ_: ans = d1+d2; break;
				  case AwkTuples._MINUS_EQ_: ans = d1-d2; break;
				  case AwkTuples._MULT_EQ_: ans = d1*d2; break;
				  case AwkTuples._DIV_EQ_: ans = d1/d2; break;
				  case AwkTuples._MOD_EQ_: ans = d1%d2; break;
				  case AwkTuples._POW_EQ_: ans = Math.pow(d1,d2); break;
				  default: throw new Error("Invalid opcode here: "+opcode);
				}
				if (ans == (int) ans) {
					push((int) ans);
			  ...
			  case AwkTuples._POW_EQ_INPUT_FIELD_:
			  {
				...
				double num;
				switch(opcode) {
				  case AwkTuples._PLUS_EQ_INPUT_FIELD_: num = JRT.toDouble(num_obj) + incval; break;
				  case AwkTuples._MINUS_EQ_INPUT_FIELD_: num = JRT.toDouble(num_obj) - incval; break;
				  case AwkTuples._MULT_EQ_INPUT_FIELD_: num = JRT.toDouble(num_obj) * incval; break;
				  case AwkTuples._DIV_EQ_INPUT_FIELD_: num = JRT.toDouble(num_obj) / incval; break;
				  case AwkTuples._MOD_EQ_INPUT_FIELD_: num = JRT.toDouble(num_obj) % incval; break;
				  case AwkTuples._POW_EQ_INPUT_FIELD_: num = Math.pow(JRT.toDouble(num_obj),incval); break;
				  default: throw new Error("Invalid opcode here: "+opcode);
				}
				String num_string;
				if (num == (int) num)
			  ...
			  case AwkTuples._SRAND_: {
				...
				if (numargs == 0) {
					...
					Object o = pop();
					if (o instanceof Double) seed = ((Double)o).intValue();
					else if (o instanceof Integer) seed = ((Integer)o).intValue();
					else {
						try {
  ...
  private void inc(int offset, boolean is_global) {
	...
	if (o == null) runtime_stack.setVariable(offset, o = BLANK, is_global);
  ...
  public void resetFNR() { runtime_stack.setVariable(fnr_offset, ZERO, true); }
  public void incFNR() { inc(fnr_offset, true); }
  public void incNR() { inc(nr_offset, true); }
  public void setNF(Integer I) { runtime_stack.setVariable(nf_offset, I, true); }
  public void setFILENAME(String filename) { runtime_stack.setVariable(filename_offset, filename, true); }
  public Object getARGV() { return runtime_stack.getVariable(argv_offset, true); }
  public Object getARGC() { return runtime_stack.getVariable(argc_offset, true); }
