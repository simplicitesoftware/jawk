...
public class AwkCompilerImpl implements AwkCompiler {
  ...
  throws IllegalArgumentException {
	...
	if (classname.length() == 0) throw new IllegalArgumentException("classname cannot be black");
	...
	for (int i=1;i<classname.length();i++)
		if (classname.charAt(i) != '.' && ! Character.isJavaIdentifierPart(classname.charAt(i)))
			throw new IllegalArgumentException("classname is not a valid java identifier");
	// - check for $
	if (classname.indexOf("$") >= 0) throw new IllegalArgumentException("classname cannot contain a $");
	// - check for no ..'s in classname
	if (classname.indexOf("..") >= 0) throw new IllegalArgumentException("null-package (..) found in classname");
...
il.append(factory.createFieldAccess(classname, "exit_code", Type.INT, Constants.GETFIELD));
  ...
  private void postcompile(AwkTuples tuples) {
	...
	try {
		...
		if (VERBOSE) System.err.println("(wrote: "+cg+")");
...
mainclass_arg.setStart(ih);
  ...
  private void getOffsets(AwkTuples tuples) {
	...
	while(! position.isEOF()) {
		int opcode = position.opcode();
		switch(opcode) {
		  case AwkTuples._NF_OFFSET_: { nf_field = "global_"+position.intArg(0); break; }
		  ...
		  case AwkTuples._RSTART_OFFSET_: { rstart_field = "global_"+position.intArg(0); break; }
		  case AwkTuples._RLENGTH_OFFSET_: { rlength_field = "global_"+position.intArg(0); break; }
		  case AwkTuples._FILENAME_OFFSET_: { filename_field = "global_"+position.intArg(0); break; }
		  ...
		  case AwkTuples._ARGC_OFFSET_: { argc_field = "global_"+(argc_offset = position.intArg(0)); break; }
...
//JVMTools_DEBUG(argc_offset, true);	// true = is global
  ...
  private final void translateToJVM(PositionForCompilation position, int opcode, AwkTuples tuples) {
	switch(opcode) {
	  ...
	  case AwkTuples._PUSH_: {
		...
		if (false) ;
		else if (arg instanceof Integer) JVMTools_pushInteger(((Integer) arg).intValue());
		else if (arg instanceof Double) JVMTools_pushDouble(((Double) arg).doubleValue());
		else if (arg instanceof String) JVMTools_pushString(arg.toString());
		else
	  ...
	  case AwkTuples._PRINT_TO_PIPE_:
	  {
		...
		for (int i=0;i<num_args;i++) {
			// stack contains objects
			JVMTools_SWAP(); JVMTools_toAwkStringForOutput(); JVMTools_SWAP();
	  ...
	  case AwkTuples._POW_EQ_:
	  {
		...
		*/
		...
		switch(opcode) {
		  ...
		  case AwkTuples._POW_EQ_: JVMTools_invokeStatic(Double.TYPE, Math.class, "pow", Double.TYPE, Double.TYPE);
					   break;
	  ...
	  case AwkTuples._POW_EQ_ARRAY_: {
		...
		switch(opcode) {
		  ...
		  case AwkTuples._POW_EQ_ARRAY_: JVMTools_invokeStatic(Double.TYPE, Math.class, "pow", Double.TYPE, Double.TYPE);
					   break;
		  default: throw new Error("Unknown opcode: "+AwkTuples.toOpcodeString(opcode));
		}
		JVMTools_fromDoubleToNumber();
  ...
  private final BranchHandle JVMTools_GOTO() { return il.append(new GOTO(null)); }
  ...
  private final InstructionHandle JVMTools_NOP() { return il.append(InstructionConstants.NOP); }
  private final InstructionHandle JVMTools_SWAP() { return il.append(InstructionConstants.SWAP); }
  private final InstructionHandle JVMTools_POP() { return il.append(InstructionConstants.POP); }
  private final InstructionHandle JVMTools_POP2() { return il.append(InstructionConstants.POP2); }
  private final InstructionHandle JVMTools_DUP() { return il.append(InstructionConstants.DUP); }
  private final InstructionHandle JVMTools_DUP_X1() { return il.append(InstructionConstants.DUP_X1); }
  private final InstructionHandle JVMTools_DUP_X2() { return il.append(InstructionConstants.DUP_X2); }
  private final void JVMTools_DUP2() { il.append(InstructionConstants.DUP2); }
  private final void JVMTools_DUP2_X1() { il.append(InstructionConstants.DUP2_X1); }
  private final void JVMTools_DUP2_X2() { il.append(InstructionConstants.DUP2_X2); }
  private final InstructionHandle JVMTools_D2I() { return il.append(InstructionConstants.D2I); }
  private final InstructionHandle JVMTools_I2D() { return il.append(InstructionConstants.I2D); }
  private final void JVMTools_D2L() { il.append(InstructionConstants.D2L); }
  private final void JVMTools_IADD() { il.append(InstructionConstants.IADD); }
  private final void JVMTools_ISUB() { il.append(InstructionConstants.ISUB); }
  private final void JVMTools_DADD() { il.append(InstructionConstants.DADD); }
  private final void JVMTools_DSUB() { il.append(InstructionConstants.DSUB); }
  private final void JVMTools_DMUL() { il.append(InstructionConstants.DMUL); }
  private final void JVMTools_DDIV() { il.append(InstructionConstants.DDIV); }
  private final void JVMTools_DREM() { il.append(InstructionConstants.DREM); }
  private final void JVMTools_DNEG() { il.append(InstructionConstants.DNEG); }
  private final void JVMTools_LMUL() { il.append(InstructionConstants.LMUL); }
  private final void JVMTools_returnVoid() { il.append(factory.createReturn(org.apache.bcel.generic.Type.VOID)); }
...
if (offset < 0) throw new IllegalArgumentException("offset = "+offset+" ?!  is_global="+is_global+", is_array="+is_array);
  ...
  private static org.apache.bcel.generic.Type getObjectType(Class cls) {
	if (cls.isArray()) return new ArrayType(getObjectType(cls.getComponentType()), 1);
	else if (cls == Boolean.TYPE) return(org.apache.bcel.generic.Type.BOOLEAN);
	else if (cls == Byte.TYPE) return(org.apache.bcel.generic.Type.BYTE);
	else if (cls == Character.TYPE) return(org.apache.bcel.generic.Type.CHAR);
	else if (cls == Double.TYPE) return(org.apache.bcel.generic.Type.DOUBLE);
	else if (cls == Float.TYPE) return(org.apache.bcel.generic.Type.FLOAT);
	else if (cls == Integer.TYPE) return(org.apache.bcel.generic.Type.INT);
	else if (cls == Long.TYPE) return(org.apache.bcel.generic.Type.LONG);
	else if (cls == Short.TYPE) return(org.apache.bcel.generic.Type.SHORT);
	else if (cls == String.class) return(org.apache.bcel.generic.Type.STRING);
	else if (cls == StringBuffer.class) return(org.apache.bcel.generic.Type.STRINGBUFFER);
	else if (cls == Object.class) return(org.apache.bcel.generic.Type.OBJECT);
	else if (cls == Void.TYPE) return(org.apache.bcel.generic.Type.VOID);
