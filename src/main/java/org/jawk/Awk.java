...
import org.jawk.backend.AwkInterpreter;
...
public class Awk {
  ...
  throws IOException, ClassNotFoundException {
	...
	try {
		...
		if (parameters.user_extensions) {
			...
			if (VERBOSE) System.err.println("(user extensions = "+extensions.keySet()+")");
		} else {
  ...
  private static int attemptToCompile(AwkParameters parameters, AwkTuples tuples) {
	try {
		if (VERBOSE) System.err.println("(locating AwkCompilerImpl...)");
		Class<?> compiler_class = Class.forName("org.jawk.backend.AwkCompilerImpl");
		if (VERBOSE) System.err.println("(found: "+compiler_class+")");
		try {
			Constructor constructor = compiler_class.getConstructor(AwkParameters.class);
			try {
				if (VERBOSE) System.err.println("(allocating new instance of the AwkCompiler class...)");
				AwkCompiler compiler = (AwkCompiler) constructor.newInstance(parameters);
				if (VERBOSE) System.err.println("(allocated: "+compiler+")");
				if (VERBOSE) System.err.println("(compiling...)");
  ...
  private static int attemptToExecuteCompiledResult(AwkParameters parameters) {
	...
	try {
		if (VERBOSE) System.err.println("(locating "+classname+"...)");
		Class<?> script_class;
		String dest_directory = parameters.destDirectory();
		if (dest_directory == null) {
			script_class = Class.forName(classname);
			if (VERBOSE) System.err.println("(found: "+script_class+")");
		} else {
			ClassLoader cl = new DestDirClassLoader(dest_directory);
			script_class = cl.loadClass(classname);
			if (VERBOSE) System.err.println("(found: "+script_class+" in "+dest_directory+")");
		}
		try {
			Constructor constructor = script_class.getConstructor();
			try {
				if (VERBOSE) System.err.println("(allocating and executing new instance of "+classname+" class...)");
  ...
  private static Map<String,JawkExtension> getJawkExtensions() {
	...
	while(st.hasMoreTokens()) {
		...
		if (VERBOSE) System.out.println("{cls = "+cls+"}");
		try {
