...
import org.jawk.backend.AVM;	// for NULL_OFFSET
...
public class AwkParser {
  ...
  private class LexerException extends IOException {
	LexerException(String msg) { super(msg+" (line "+reader.getLineNumber()+")"); }
  }
...
////				read();
	...
	if (c == '(') { read(); return token = _OPEN_PAREN_; }
	if (c == ')') { read(); return token = _CLOSE_PAREN_; }
	if (c == '{') { read(); return token = _OPEN_BRACE_; }
	if (c == '}') { read(); return token = _CLOSE_BRACE_; }
	if (c == '[') { read(); return token = _OPEN_BRACKET_; }
	if (c == ']') { read(); return token = _CLOSE_BRACKET_; }
	if (c=='$') { read(); return token = _DOLLAR_ ; }
	if (c=='~') { read(); return token = _MATCHES_; }
	if (c=='?') { read(); return token = _QUESTION_MARK_; }
	if (c==':') { read(); return token = _COLON_; }
	if (c=='&') {
		read();
		if (c=='&') {
			read();
	...
	if (Character.isJavaIdentifierStart(c)) {
		...
		if (extensions.get(text.toString()) != null) return token = _EXTENSION_;
		Integer kw_token = KEYWORDS.get(text.toString());
		if (kw_token != null) return token = kw_token.intValue();
		Integer bf_idx = BUILTIN_FUNC_NAMES.get(text.toString());
		if (bf_idx != null) return token = _BUILTIN_FUNC_NAME_;
		if (c == '(')
  ...
  throws IOException {
	...
	if (token == KEYWORDS.get("BEGIN")) {
		...
		opt_expr = ASSIGNMENT_EXPRESSION(true,true,false);	// true = allow comparators, allow IN keyword, do NOT allow multidim indices expressions
		// for /regex/, /regex/
		if (token == _COMMA_) {
			lexer();
			opt_newline();
			opt_expr = new RegexpPair_AST(opt_expr, ASSIGNMENT_EXPRESSION(true,true,false));	// true = allow comparators, allow IN keyword, do NOT allow multidim indices expressions
		}
	} else
		opt_expr = null;
  ...
  throws IOException {
	...
	if (pre_inc) return new PreInc_AST(factor_ast);
	else if (pre_dec) return new PreDec_AST(factor_ast);
	else if (post_inc) return new PostInc_AST(factor_ast);
	else if (post_dec) return new PostDec_AST(factor_ast);
	else return factor_ast;
  }
  ...
  throws IOException {
	...
	else if (additional_type_functions && token == KEYWORDS.get("_INTEGER")) return INTEGER_EXPRESSION(allow_comparators, allow_in_keyword, allow_multidim_indices);
	else if (additional_type_functions && token == KEYWORDS.get("_DOUBLE")) return DOUBLE_EXPRESSION(allow_comparators, allow_in_keyword, allow_multidim_indices);
	else if (additional_type_functions && token == KEYWORDS.get("_STRING")) return STRING_EXPRESSION(allow_comparators, allow_in_keyword, allow_multidim_indices);
	else {
		AST target_ast;
		if (token == _DOLLAR_) {
			lexer();
  ...
  throws IOException {
	if (token != _ID_ && token != _FUNC_ID_ && token != _BUILTIN_FUNC_NAME_ && token != _EXTENSION_) throw new ParserException("Expecting an ID.  Got ("+token+"): "+text);
	int id_token = token;
	String id = text.toString();
	boolean parens = (c == '(');
	lexer();
  ...
  throws IOException {
	...
	if (token == KEYWORDS.get("in")) {
		if (expr1.ast1 == null || expr1.ast2 != null) throw new ParserException("Invalid expression prior to 'in' statement.  Got : "+expr1);
		expr1 = expr1.ast1;
		// analyze expr1 to make sure it's a singleton ID_AST
		if (expr1 == null || ! (expr1 instanceof ID_AST)) throw new ParserException("Expecting an ID for 'in' statement.  Got : "+expr1);
		// in
		lexer();
		// id
		if (token != _ID_) throw new ParserException("Expecting an ARRAY ID for 'in' statement.  Got ("+token+"): "+text);
	...
	if (token == _SEMICOLON_) lexer(); else throw new ParserException("Expecting ;.  Got ("+token+"): "+text);
	if (token != _SEMICOLON_)
  ...
  throws IOException {
	if (token == _SEMICOLON_ ) return null;
	else if (token == KEYWORDS.get("delete")) return DELETE_STATEMENT();
	else if (token == KEYWORDS.get("print")) return PRINT_STATEMENT();
	else if (token == KEYWORDS.get("printf")) return PRINTF_STATEMENT();
	else return EXPRESSION_STATEMENT(allow_in_keyword, true);	// allow NonStatement_ASTs
  }
  ...
  throws IOException {
	...
	if (token == _DOLLAR_) return FACTOR(allow_comparators, allow_in_keyword, false);
	if (token == _ID_) return FACTOR(allow_comparators, allow_in_keyword, false);
	return null;
  }
  ...
  private abstract class AST implements AwkSyntaxTree {
	...
	private void dump(PrintStream ps, int lvl) {
		...
		if (ast1 != null) ast1.dump(ps, lvl+1);
		if (ast2 != null) ast2.dump(ps, lvl+1);
		if (ast3 != null) ast3.dump(ps, lvl+1);
		if (ast4 != null) ast4.dump(ps, lvl+1);
	}
	...
	throws SemanticException {
		if (ast1 != null) ast1.semanticAnalysis();
		if (ast2 != null) ast2.semanticAnalysis();
		if (ast3 != null) ast3.semanticAnalysis();
		if (ast4 != null) ast4.semanticAnalysis();
	}
	...
	private boolean isBegin() {
		...
		if (!result && ast1 != null) result = ast1.isBegin();
		if (!result && ast2 != null) result = ast2.isBegin();
		if (!result && ast3 != null) result = ast3.isBegin();
		if (!result && ast4 != null) result = ast4.isBegin();
		return result;
	}
	...
	private boolean isEnd() {
		...
		if (!result && ast1 != null) result = ast1.isEnd();
		if (!result && ast2 != null) result = ast2.isEnd();
		if (!result && ast3 != null) result = ast3.isEnd();
		if (!result && ast4 != null) result = ast4.isEnd();
		return result;
	}
	...
	private boolean isFunction() {
		...
		if (!result && ast1 != null) result = ast1.isFunction();
		if (!result && ast2 != null) result = ast2.isFunction();
		if (!result && ast3 != null) result = ast3.isFunction();
		if (!result && ast4 != null) result = ast4.isFunction();
		return result;
	}
	...
	protected class SemanticException extends RuntimeException {
		SemanticException(String msg) { super(msg+" (line: "+lineno+")"); }
	}
  ...
  private abstract class ScalarExpression_AST extends AST {
	protected ScalarExpression_AST() { super(); }
  ...
  */
  ...
  private static boolean containsASTType(AST ast, Class[] cls_array) {
	...
	for(Class cls : cls_array)
		if (cls.isInstance(ast))
			return true;
	if (containsASTType(ast.ast1, cls_array)) return true;
	if (containsASTType(ast.ast2, cls_array)) return true;
	if (containsASTType(ast.ast3, cls_array)) return true;
	if (containsASTType(ast.ast4, cls_array)) return true;
	return false;
  }

  private Address next_address;

  private class RuleList_AST extends AST {

	private RuleList_AST(AST rule, AST rest) { super(rule, rest); }
  ...
  private class Rule_AST extends AST implements Nextable {
  ...
  private class IfStatement_AST extends AST {
	private IfStatement_AST(AST expr, AST b1, AST b2) { super(expr, b1, b2); }
	public int populateTuples(AwkTuples tuples) {
		pushSourceLineNumber(tuples);
		assert ast1 != null;
  ...
  private class TertiaryExpression_AST extends ScalarExpression_AST {
	private TertiaryExpression_AST(AST a1, AST a2, AST a3) { super(a1,a2,a3); }
	public int populateTuples(AwkTuples tuples) {
		pushSourceLineNumber(tuples);
		assert ast1!=null;
		assert ast2!=null;
  ...
  private class WhileStatement_AST extends AST implements Breakable, Continueable {
	...
	private WhileStatement_AST(AST expr, AST block) { super(expr, block); }
	public int populateTuples(AwkTuples tuples) {
		pushSourceLineNumber(tuples);
  ...
  private class DoStatement_AST extends AST implements Breakable, Continueable {
	...
	private DoStatement_AST(AST block, AST expr) { super(block, expr); }
	public int populateTuples(AwkTuples tuples) {
		pushSourceLineNumber(tuples);
  ...
  private class ForStatement_AST extends AST implements Breakable, Continueable {
	...
	private ForStatement_AST(AST expr1, AST expr2, AST expr3, AST block) { super(expr1, expr2, expr3, block); }
	public int populateTuples(AwkTuples tuples) {
		pushSourceLineNumber(tuples);
  ...
  private class ForInStatement_AST extends AST implements Breakable, Continueable {
	...
	private ForInStatement_AST(AST key_id_ast, AST array_id_ast, AST block) { super(key_id_ast, array_id_ast, block); }
	public int populateTuples(AwkTuples tuples) {
		pushSourceLineNumber(tuples);
  ...
  private class EmptyStatement_AST extends AST {
	private EmptyStatement_AST() { super(); }
	public int populateTuples(AwkTuples tuples) {
		pushSourceLineNumber(tuples);
		// nothing to populate!
		popSourceLineNumber(tuples);
   ...
   */
  private class ExpressionStatement_AST extends AST {
	private ExpressionStatement_AST(AST expr) { super(expr); }
	public int populateTuples(AwkTuples tuples) {
		pushSourceLineNumber(tuples);
  ...
  private class AssignmentExpression_AST extends ScalarExpression_AST {
	/** operand / operator */
	private int op;
	private String text;
	private AssignmentExpression_AST(AST lhs, int op, String text, AST rhs) { super(lhs, rhs); this.op = op; this.text = text; }
	public String toString() { return super.toString()+" ("+op+"/"+text+")"; }
	public int populateTuples(AwkTuples tuples) {
		pushSourceLineNumber(tuples);
		assert ast2 != null;
		int ast2_count = ast2.populateTuples(tuples);
		...
		if (ast1 instanceof ID_AST) {
			...
			else if (op == _PLUS_EQ_) tuples.plusEq(id_ast.offset, id_ast.is_global);
			else if (op == _MINUS_EQ_) tuples.minusEq(id_ast.offset, id_ast.is_global);
			else if (op == _MULT_EQ_) tuples.multEq(id_ast.offset, id_ast.is_global);
			else if (op == _DIV_EQ_) tuples.divEq(id_ast.offset, id_ast.is_global);
			else if (op == _MOD_EQ_) tuples.modEq(id_ast.offset, id_ast.is_global);
			else if (op == _POW_EQ_) tuples.powEq(id_ast.offset, id_ast.is_global);
			else
			...
			if (false) ;
			else if (op == _EQUALS_) tuples.assignArray(id_ast.offset, id_ast.is_global);
			else if (op == _PLUS_EQ_) tuples.plusEqArray(id_ast.offset, id_ast.is_global);
			else if (op == _MINUS_EQ_) tuples.minusEqArray(id_ast.offset, id_ast.is_global);
			else if (op == _MULT_EQ_) tuples.multEqArray(id_ast.offset, id_ast.is_global);
			else if (op == _DIV_EQ_) tuples.divEqArray(id_ast.offset, id_ast.is_global);
			else if (op == _MOD_EQ_) tuples.modEqArray(id_ast.offset, id_ast.is_global);
			else if (op == _POW_EQ_) tuples.powEqArray(id_ast.offset, id_ast.is_global);
			else
			...
			else if (op == _PLUS_EQ_) tuples.plusEqInputField();
			else if (op == _MINUS_EQ_) tuples.minusEqInputField();
			else if (op == _MULT_EQ_) tuples.multEqInputField();
			else if (op == _DIV_EQ_) tuples.divEqInputField();
			else if (op == _MOD_EQ_) tuples.modEqInputField();
			else if (op == _POW_EQ_) tuples.powEqInputField();
			else
  ...
  private class InExpression_AST extends ScalarExpression_AST {
	private InExpression_AST(AST arg, AST arr) { super(arg, arr); }
	public int populateTuples(AwkTuples tuples) {
		pushSourceLineNumber(tuples);
		assert ast1 != null;
		assert ast2 != null;
  ...
  private class ComparisonExpression_AST extends ScalarExpression_AST {
	/** operand / operator */
	private int op;
	private String text;
	private ComparisonExpression_AST(AST lhs, int op, String text, AST rhs) { super(lhs, rhs); this.op = op; this.text = text; }
	public String toString() { return super.toString()+" ("+op+"/"+text+")"; }
	public int populateTuples(AwkTuples tuples) {
		pushSourceLineNumber(tuples);
		assert ast1 != null;
  ...
  private class LogicalExpression_AST extends ScalarExpression_AST {
	/** operand / operator */
	private int op;
	private String text;
	private LogicalExpression_AST(AST lhs, int op, String text, AST rhs) { super(lhs, rhs); this.op = op; this.text = text; }
	public String toString() { return super.toString()+" ("+op+"/"+text+")"; }
	public int populateTuples(AwkTuples tuples) {
		pushSourceLineNumber(tuples);
  ...
  private class BinaryExpression_AST extends ScalarExpression_AST {
	/** operand / operator */
	private int op;
	private String text;
	private BinaryExpression_AST(AST lhs, int op, String text, AST rhs) { super(lhs, rhs); this.op = op; this.text = text; }
	public String toString() { return super.toString()+" ("+op+"/"+text+")"; }
	public int populateTuples(AwkTuples tuples) {
		pushSourceLineNumber(tuples);
		int ast1_result = ast1.populateTuples(tuples);
		...
		tuples.swap();
		if (false) ;
		else if (op == _PLUS_) tuples.add();
		else if (op == _MINUS_) tuples.subtract();
		else if (op == _MULT_) tuples.multiply();
		else if (op == _DIVIDE_) tuples.divide();
		else if (op == _MOD_) tuples.mod();
		else if (op == _POW_) tuples.pow();
		else throw new Error("Unhandled op: "+op+" / "+this);
		popSourceLineNumber(tuples);
		return 1;
	}
  }

  private class ConcatExpression_AST extends ScalarExpression_AST {
	private ConcatExpression_AST(AST lhs, AST rhs) { super(lhs, rhs); }
	public int populateTuples(AwkTuples tuples) {
		pushSourceLineNumber(tuples);
		assert ast2 != null;
  ...
  private class NegativeExpression_AST extends ScalarExpression_AST {
	private NegativeExpression_AST(AST expr) { super(expr); }
	public int populateTuples(AwkTuples tuples) {
		pushSourceLineNumber(tuples);
		assert ast1 != null;
		int ast1_result = ast1.populateTuples(tuples);
  ...
  private class NotExpression_AST extends ScalarExpression_AST {
	private NotExpression_AST(AST expr) { super(expr); }
	public int populateTuples(AwkTuples tuples) {
		pushSourceLineNumber(tuples);
		assert ast1 != null;
		int ast1_result = ast1.populateTuples(tuples);
  ...
  private class DollarExpression_AST extends ScalarExpression_AST {
	private DollarExpression_AST(AST expr) { super(expr); }
	public int populateTuples(AwkTuples tuples) {
		pushSourceLineNumber(tuples);
		assert ast1 != null;
		int ast1_result = ast1.populateTuples(tuples);
  ...
  private class ArrayIndex_AST extends ScalarExpression_AST {
	private ArrayIndex_AST(AST expr_ast, AST next) { super(expr_ast, next); }
	public int populateTuples(AwkTuples tuples) {
		pushSourceLineNumber(tuples);
		AST ptr = this;
		int cnt=0;
  ...
  private class STATEMENTLIST_AST extends AST {
	private STATEMENTLIST_AST(AST statement_ast, AST rest) { super(statement_ast, rest); }
	/**
	...
	public String toString() { return super.toString()+" <"+ast1+">"; }
  }
  ...
  private class FunctionDef_AST extends AST implements Returnable {
	private String id;
	...
	private FunctionDef_AST(String id, AST params, AST func_body) { super(params, func_body); this.id = id; is_function = true; }
  ...
  private class FunctionCall_AST extends ScalarExpression_AST {
	private FunctionProxy function_proxy;
	private FunctionCall_AST(FunctionProxy function_proxy, AST params) { super(params); this.function_proxy = function_proxy; }
	/**
  ...
  private class BuiltinFunctionCall_AST extends ScalarExpression_AST {
	private String id;
	private int f_idx;
  ...
  private class FunctionCallParamList_AST extends AST {
	private FunctionCallParamList_AST(AST expr, AST rest) { super(expr, rest); }
	public int populateTuples(AwkTuples tuples) {
		pushSourceLineNumber(tuples);
		assert ast1 != null;
		int retval;
  ...
  private class FunctionDefParamList_AST extends AST {
	private String id;
	private int offset;
	private FunctionDefParamList_AST(String id, int offset, AST rest) { super(rest); this.id = id; this.offset = offset; }
  ...
  private class ID_AST extends AST implements NonStatement_AST {
	private String id;
	private int offset = AVM.NULL_OFFSET;
	private boolean is_global;
	private ID_AST(String id, boolean is_global) { this.id = id; this.is_global = is_global; }
	...
	public final boolean isArray() { return is_array; }
	public final boolean isScalar() { return is_scalar; }

	private void setArray(boolean b) { is_array = b; }
	private void setScalar(boolean b) { is_scalar = b; }
  }

  private class ArrayReference_AST extends ScalarExpression_AST {
	private ArrayReference_AST(AST id_ast, AST idx_ast) { super(id_ast, idx_ast); }
	public String toString() { return super.toString()+" ("+ast1+" [...])"; }
	public int populateTuples(AwkTuples tuples) {
		pushSourceLineNumber(tuples);
		assert ast1 != null;
  ...
  private class Integer_AST extends ScalarExpression_AST implements NonStatement_AST {
	private Integer I;
	private Integer_AST(Integer I) { this.I = I; }
   ...
   */
  private class Double_AST extends ScalarExpression_AST implements NonStatement_AST {
	private Object D;
   ...
   */
  private class String_AST extends ScalarExpression_AST implements NonStatement_AST {
	private String S;
  ...
  private class RegexpPair_AST extends ScalarExpression_AST {
	private RegexpPair_AST(AST regexp_ast_1, AST regexp_ast_2) { super(regexp_ast_1, regexp_ast_2); }
	public int populateTuples(AwkTuples tuples) {
		pushSourceLineNumber(tuples);
		assert ast2 != null;
		int ast2_result = ast2.populateTuples(tuples);
  ...
  private class IntegerExpression_AST extends ScalarExpression_AST {
	private IntegerExpression_AST(AST expr) { super(expr); }
	public int populateTuples(AwkTuples tuples) {
		pushSourceLineNumber(tuples);
		int ast1_result = ast1.populateTuples(tuples);
		assert ast1_result == 1;
  ...
  private class DoubleExpression_AST extends ScalarExpression_AST {
	private DoubleExpression_AST(AST expr) { super(expr); }
	public int populateTuples(AwkTuples tuples) {
		pushSourceLineNumber(tuples);
		int ast1_result = ast1.populateTuples(tuples);
		assert ast1_result == 1;
  ...
  private class StringExpression_AST extends ScalarExpression_AST {
	private StringExpression_AST(AST expr) { super(expr); }
	public int populateTuples(AwkTuples tuples) {
		pushSourceLineNumber(tuples);
		int ast1_result = ast1.populateTuples(tuples);
		assert ast1_result == 1;
  ...
  private class Begin_AST extends AST {
	private Begin_AST() { super(); is_begin = true; }
	public int populateTuples(AwkTuples tuples) {
		pushSourceLineNumber(tuples);
		tuples.push(1);
		popSourceLineNumber(tuples);
  ...
  private class End_AST extends AST {
	private End_AST() { super(); is_end = true; }
	public int populateTuples(AwkTuples tuples) {
		pushSourceLineNumber(tuples);
		tuples.push(1);
		popSourceLineNumber(tuples);
  ...
  private class PreInc_AST extends ScalarExpression_AST {
  ...
  private class PreDec_AST extends ScalarExpression_AST {
  ...
  private class PostInc_AST extends ScalarExpression_AST {
  ...
  private class PostDec_AST extends ScalarExpression_AST {
  ...
  private class Print_AST extends ScalarExpression_AST {
  ...
  private class Extension_AST extends AST {
	...
	public String toString() { return super.toString()+" ("+extension_keyword+")"; }
  }

  private class Printf_AST extends ScalarExpression_AST {
	private int output_token;
  ...
  private class Getline_AST extends ScalarExpression_AST {
  ...
  private class ReturnStatement_AST extends AST {
	private ReturnStatement_AST(AST expr) { super(expr); }
	public int populateTuples(AwkTuples tuples) {
		pushSourceLineNumber(tuples);
  ...
  private class ExitStatement_AST extends AST {
	private ExitStatement_AST(AST expr) { super(expr); }
	public int populateTuples(AwkTuples tuples) {
		pushSourceLineNumber(tuples);
		if (ast1 != null) {
			int ast1_result = ast1.populateTuples(tuples);
  ...
  private class DeleteStatement_AST extends AST {
	private DeleteStatement_AST(AST symbol_ast) { super(symbol_ast); }
	public int populateTuples(AwkTuples tuples) {
		pushSourceLineNumber(tuples);
		assert ast1 != null;
  ...
  private class SleepStatement_AST extends AST {
	private SleepStatement_AST(AST expr) { super(expr); }
	public int populateTuples(AwkTuples tuples) {
		pushSourceLineNumber(tuples);
		if (ast1 == null)
			tuples.sleep(0);
  ...
  private class DumpStatement_AST extends AST {
	private DumpStatement_AST(AST expr) { super(expr); }
	public int populateTuples(AwkTuples tuples) {
		pushSourceLineNumber(tuples);
		if (ast1 == null)
			tuples.dump(0);
  ...
  private class ContinueStatement_AST extends AST {
  ...
  private class FunctionProxy implements HasFunctionAddress {
	private FunctionDef_AST function_def_ast;
	private String id;
	private FunctionProxy(String id) { this.id = id; }
	...
	private boolean isDefined() { return function_def_ast != null; }
	public Address getFunctionAddress() { return function_def_ast.getAddress(); }
	private String getFunctionName() { return id; }
	private int getFunctionParamCount() { return function_def_ast.paramCount(); }
	public String toString() { return super.toString()+" ("+id+")"; }
  ...
  private class AwkSymbolTableImpl {
	...
	int numGlobals() { return global_ids.size(); }
	...
	AST addDOUBLE(String dbl) { return new Double_AST(Double.valueOf(dbl)); }
	AST addSTRING(String str) { return new String_AST(str); }
	AST addREGEXP(String regexp) { return new Regexp_AST(regexp); }
  } // private class AwkSymbolTableImpl

  private class ParserException extends RuntimeException {
	ParserException(String msg) { super(msg+" (line "+reader.getLineNumber()+")"); }
  }
} // public class AwkParser
