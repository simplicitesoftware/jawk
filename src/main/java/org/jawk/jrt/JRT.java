...
public class JRT {
  ...
  public final boolean toBoolean(Object o) {
	boolean val;
	if (o instanceof Integer) val = ((Integer)o).intValue() != 0;
	else if (o instanceof Double) val = ((Double)o).doubleValue() != 0;
	else if (o instanceof String) val = (o.toString().length()>0);
   ...
   */
  public String jrtGetInputString() { return jrt_input_string; }
  ...
  public void jrtCloseAll() {
	...
	for (String s : file_readers.keySet()) set.add(s);
	for (String s : command_readers.keySet()) set.add(s);
	for (String s : output_files.keySet()) set.add(s);
	for (String s : output_streams.keySet()) set.add(s);
	for (String s : set) { jrtClose(s); }
  }
  ...
  public static java.util.Random newRandom(int seed) { return new java.util.Random(seed); }
